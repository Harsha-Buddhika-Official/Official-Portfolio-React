  ...
  return (
    <div className=" bg-[#1a1a1a88] text-white px-3 py-6 md:py-1 my-0 w-full">
      <div className="flex items-center justify-between px-4 mx-auto max-w-7xl sm:px-2 lg:px-8">
        ...
        <nav
          className={`flex-col md:flex-row md:flex gap-3 absolute md:static top-16 left-0 w-full md:w-auto bg-[#1a1a1a] md:bg-transparent z-10 transition-all duration-300 ${
            menuOpen ? "flex" : "hidden"
          } py-4 md:gap-5`}
        >
          {["Home", "About", "Project", "Skills", "Education", "Contacts"].map(
            (section) => (
              <button
                key={section}
              ...
              >
                <span
                  className={`navi ${
                    activeSection === section ? "navi-active" : ""
                  }`}
                >
                  {section}
                </span>
